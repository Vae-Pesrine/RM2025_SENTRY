...
// 发布世界坐标系下的点云数据，并根据需要保存点云数据到PCD文件
...
//------------------------------------------------------------------------------------------------------
    ...
    {
        string all_points_dir(string(string(ROOT_DIR) + "PCD/") + pcd_file_name);
        pcl::PCDWriter pcd_writer;
        pcd_writer.writeBinary(all_points_dir, *pcl_wait_save);
    }
    fout_out.close();
