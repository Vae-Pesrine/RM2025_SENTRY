...
void LivoxPointsPlugin::OnNewLaserScans() {
    if (rayShape) {
        ...
        livox_ros_driver2::CustomMsg livox;
        int count = 0;
        ...
        for(auto &pair : points_pair)
        {
            ...
            livox_ros_driver2::CustomPoint livox_point;
            livox_point.x = point.X();
            livox_point.y = point.Y();
            livox_point.z = point.Z();
            livox_point.reflectivity = intensity;

            std::chrono::steady_clock::time_point end_time = std::chrono::steady_clock::now();
            std::chrono::duration<double> time_used = end_time - start_time;
            livox_point.offset_time = time_used.count();
            livox.points.push_back(livox_point);
            count++;
        }
        livox.point_num = count;
        livox.header.stamp = ros::Time::now();
        livox.header.frame_id = raySensor->Name();
        rosPointPub.publish(livox);
        ros::spinOnce();
