...
: pr_nh_("~")
{
  pr_nh_.param<std::string>("sensor_frame", sensor_frame_);
  pr_nh_.param<double>("scan_voxel_size", scan_voxel_size_);
  pr_nh_.param<double>("decay_time", decay_time_);
  pr_nh_.param<double>("no_decay_dis", no_decay_dis_);
  pr_nh_.param<double>("clearing_dis", clearing_dis_);
  pr_nh_.param<bool>("use_sorting", use_sorting_);
  pr_nh_.param<double>("quantile_z", quantile_z_);
  pr_nh_.param<bool>("consider_drop", consider_drop_);
  pr_nh_.param<bool>("limit_ground_lift", limit_ground_lift_);
  pr_nh_.param<double>("max_ground_lift", max_ground_lift_);
  pr_nh_.param<bool>("clear_dy_obs", clear_dy_obs_);
  pr_nh_.param<double>("min_dy_obs_dis", min_dy_obs_dis_);
  pr_nh_.param<double>("min_dy_obs_angle", min_dy_obs_angle_);
  pr_nh_.param<double>("min_dy_obs_rel_z", min_dy_obs_rel_z_);
  pr_nh_.param<double>("abs_dy_obs_rel_z_thre", abs_dy_obs_rel_z_thre_);
  pr_nh_.param<double>("min_dy_obs_vfov", min_dy_obs_vfov_);
  pr_nh_.param<double>("max_dy_obs_vfov", max_dy_obs_vfov_);
  pr_nh_.param<int>("min_dy_obs_point_num", min_dy_obs_point_num_);
  pr_nh_.param<bool>("no_data_obstacle", no_data_obstacle_);
  pr_nh_.param<int>("no_data_block_skip_num", no_data_block_skip_num_);
  pr_nh_.param<int>("min_block_point_num", min_block_point_num_);
  pr_nh_.param<double>("vehicle_height", vehicle_height_);
  pr_nh_.param<int>("voxel_point_update_thre", voxel_point_update_thre_);
  pr_nh_.param<double>("voxel_time_update_thre", voxel_time_update_thre_);
  pr_nh_.param<double>("min_rel_z", min_rel_z_);
  pr_nh_.param<double>("max_rel_z", max_rel_z_);
  pr_nh_.param<double>("dis_ratio_z", dis_ratio_z_);
  ...
  odometry_sub_ = nh_.subscribe<nav_msgs::Odometry>("/odom", 5, &TerrainAnalysisNode::odometryHandler, this);


  laser_cloud_sub_ = nh_.subscribe<sensor_msgs::PointCloud2>(
    "/cloud_registered", 5,
    &TerrainAnalysisNode::laserCloudHandler, this);

  joystick_sub_ = nh_.subscribe<sensor_msgs::Joy>(
    "/joy", 5, std::bind(&TerrainAnalysisNode::joystickHandler, this, std::placeholders::_1));
  clearing_sub_ = nh_.subscribe<std_msgs::Float32>(
    "/map_clearing", 5,
    std::bind(&TerrainAnalysisNode::clearingHandler, this, std::placeholders::_1));

  terrain_map_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("/terrain_map", 2);
...
{
  laser_cloud_time_ = ros::Time(laserCloud->header.stamp).toSec();
  if (!system_inited_) {
...
void TerrainAnalysisNode::processLaserCloud()
{
  ...
  terrain_cloud.header.stamp = ros::Time(static_cast<uint64_t>(laser_cloud_time_ * 1e9));
  terrain_cloud.header.frame_id = "odom";

  // Publish the transformed point cloud
  terrain_map_pub_.publish(terrain_cloud);
}
