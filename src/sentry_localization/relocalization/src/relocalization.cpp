...
Relocalization::Relocalization(): 
    ...
    register_ = std::make_unique<small_gicp::Registration<small_gicp::GICPFactor, small_gicp::ParallelReductionOMP>>();
    ...
    register_timer_ = this->createWallTimer(ros::WallDuration(1 / registration_frequency_), &Relocalization::runRegistration, this, false, true);
    transform_timer_ = this->createWallTimer(ros::WallDuration(1 / pose_update_frequency_), &Relocalization::publishTransform, this, false, true);
...
void Relocalization::runRegistration(
    ...
    if(debug_en_)
        std::cout << GREEN << "[GICP] time: " << align_duration.count() << " ms" 
                           << " num_inliers: " << result.num_inliers
                           << " error: " << result.error << RESET;
    if(!result.converged){
        if(debug_en_){
            ROS_WARN_STREAM("The small gicp didn't converge");
            std::cout << YELLOW << "[GICP] time: " << align_duration.count() << " ms" 
                                << " num_inliers: " << result.num_inliers 
                                << " error: " << result.error << RESET;
        }
        if(align_duration.count() > align_time_thre_) 
            if_bbs_localize_ = true;
        return;
    }
...
int main(int argc, char *argv[])
{
    ...
    Relocalization small_gicp_relocalization;
