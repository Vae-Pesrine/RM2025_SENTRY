...
private:
    ...
    std::mutex keyframes_mutex_, vis_mutex_, latest_frame_mutex_;
    bool is_initialized_ = false;
    int current_keyframe_idx_ = 0;
    PosePcdStamped last_keyframe_, latest_frame_;
    std::vector<PosePcdReduced> saved_map_from_bag_;
    Eigen::Matrix4d last_corrected_TF_ = Eigen::Matrix4d::Identity();
    ///// map match
    double keyframe_dist_thr_, keyframe_time_thr_;
    double voxel_res_;
    ///// visualize
    ...
    ros::ServiceServer manual_match_srv_;
...
public:
    explicit ScqnLocalization(const ros::NodeHandle &n_private);
    ~ScqnLocalization() {};

    bool manualMatchService(std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res);
