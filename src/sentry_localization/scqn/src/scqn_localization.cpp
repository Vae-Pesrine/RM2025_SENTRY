#include "scqn/scqn_localization.hpp"
...
void ScqnLocalization::odomPcdCallback(const nav_msgs::OdometryConstPtr &odom_msg, const sensor_msgs::PointCloud2ConstPtr &pcd_msg)
{
    PosePcdStamped current_frame = PosePcdStamped(*odom_msg, *pcd_msg, current_keyframe_idx_); // to be checked if keyframe or not
    ...
    broadcaster_.sendTransform(tf::StampedTransform(poseEigToROSTf(last_corrected_TF_),
                                                    ros::Time::now(),
                                                    map_frame_,
                                                    "odom"));
...
void ScqnLocalization::matchingTimerFunc(const ros::TimerEvent &event)
{
    if (!is_initialized_)
    {
        return;
    ...
    PosePcdStamped last_keyframe_copy;
    {
        std::lock_guard<std::mutex> lock(keyframes_mutex_);
...
void ScqnLocalization::updateOdomsAndPaths(const PosePcdStamped &pose_pcd_in)
{
...
visualization_msgs::Marker ScqnLocalization::getMatchMarker(const std::vector<std::pair<pcl::PointXYZ, pcl::PointXYZ>> &match_xyz_pairs)
{
...
bool ScqnLocalization::checkIfKeyframe(const PosePcdStamped &pose_pcd_in, const PosePcdStamped &latest_pose_pcd)
{
...
void ScqnLocalization::loadMap(const std::string &saved_map_path)
{
    rosbag::Bag bag;
...
int main(int argc, char **argv)
{
    ...
    ScqnLocalization scqn_localization_node(pr_nh);
