#include "scqn/scqn_sam.hpp"

ScqnSam::ScqnSam(const ros::NodeHandle &n_private):
    nh_(n_private)
{
    ////// ROS params
    ...
    /* Timers */
    loop_timer_ = nh_.createTimer(ros::Duration(1 / loop_update_hz), &ScqnSam::loopTimerFunc, this);
    vis_timer_ = nh_.createTimer(ros::Duration(1 / vis_hz), &ScqnSam::visTimerFunc, this);
    ROS_INFO("Main class, starting node...");
}

void ScqnSam::odomPcdCallback(const nav_msgs::OdometryConstPtr &odom_msg,
...
void ScqnSam::loopTimerFunc(const ros::TimerEvent &event)
{
...
void ScqnSam::visTimerFunc(const ros::TimerEvent &event)
{
    if (!is_initialized_)
    {
        return;
...
void ScqnSam::saveFlagCallback(const std_msgs::String::ConstPtr &msg)
{
...
void ScqnSam::updateOdomsAndPaths(const PosePcd &pose_pcd_in)
{
...
bool ScqnSam::checkIfKeyframe(const PosePcd &pose_pcd_in, const PosePcd &latest_pose_pcd)
{
...
int main(int argc, char **argv)
{
    ...
    ScqnSam scqn_sam_node(pr_nh);

    ros::AsyncSpinner spinner(4);
    spinner.start();
    ros::waitForShutdown();
    scqn_sam_node.~ScqnSam();
