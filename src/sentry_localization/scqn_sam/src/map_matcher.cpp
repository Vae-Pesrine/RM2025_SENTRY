...
void MapMatcher::updateScanContext(
...
int MapMatcher::fetchClosestKeyFrameIndex(
...
PcdPair MapMatcher::setSourceAndTargetCloud(
    const PosePcd &query_keyframe,
    ...
    pcl::PointCloud<PointType> target_accum, source_out;
    int num_approx = saved_keyframes[target_index].pcd_.size() * 2 * submap_range;
    target_accum.reserve(num_approx);

    source_out = transformPcd(query_keyframe.pcd_, query_keyframe.pose_corrected_eig_);
    if (quatro_en){
        target_accum = transformPcd(saved_keyframes[target_index].pcd_, saved_keyframes[target_index].pose_eig_);
    } else{
        // For ICP matching,
        // empirically scan-to-submap matching works better
        for (int i = target_index - submap_range; i < target_index + submap_range + 1; ++i){
            if (i >= 0 && i < static_cast<int>(saved_keyframes.size() - 1)){
                target_accum += transformPcd(saved_keyframes[i].pcd_, saved_keyframes[i].pose_eig_);
            }
        }
    }

    return {*voxelizePcd(source_out, voxel_res), *voxelizePcd(target_accum, voxel_res)};
}

RegistrationOutput MapMatcher::icpAlignment(
...
RegistrationOutput MapMatcher::coarseToFineAlignment(
    const pcl::PointCloud<PointType> &source,
    const pcl::PointCloud<PointType> &target
) {
    RegistrationOutput reg_output;
...
RegistrationOutput MapMatcher::performMapMatcher(
    const PosePcd &query_keyframe,
    const std::vector<PosePcdReduced> &saved_keyframes,
    const int closest_keyframe_index
) {
    RegistrationOutput reg_output;
    closest_keyframe_index_ = closest_keyframe_index;
    if (closest_keyframe_index_ >= 0){
        // Quatro + NANO-GICP to check loop (from front_keyframe to closest
        // keyframe's neighbor)
        const auto &[source_cloud, target_cloud] = setSourceAndTargetCloud(query_keyframe,
                                                                           saved_keyframes,
                                                                           closest_keyframe_index_,
        ...
        *target_cloud_ = target_cloud;
        ...
        if (config_.quatro_en_){
            std::cout << "\033[1;35mExecute coarse-to-fine alignment: "
                      << source_cloud.size() << " vs " << target_cloud.size() << "\033[0m\n";
            return coarseToFineAlignment(source_cloud, target_cloud);
        } else{
            std::cout << "\033[1;35mExecute GICP: " << source_cloud.size() << " vs "
                      << target_cloud.size() << "\033[0m\n";
...
int MapMatcher::getClosestKeyFrameIndex()
{
    return closest_keyframe_index_;
}