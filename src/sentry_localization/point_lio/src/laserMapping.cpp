...
//------------------------------------------------------------------------------------------------------
    ...
    {
        std::string pkg_path =  ros::package::getPath("point_lio");
        std::string pcd_path = pkg_path + "/PCD/" + pcd_file_name;
        pcl::PCDWriter pcd_writer;
        pcd_writer.writeBinary(pcd_path, *pcl_wait_save);
        std::cout << "\033[34mThe pcd file has been saved into: " << pcd_path << "\033[0m\n";
    }
    fout_out.close();
    fout_imu_pbp.close();
    return 0;
