...
public:
    LivoxConverter(){
        ...
        const auto add_field = [this](const std::string &name, const int offset, const uint8_t datatype){
            ...
            field.count = 1;
            points_msg->fields.emplace_back(field);
        };
        ...
        add_field("intensity", points_msg->fields.back().offset + sizeof(uint32_t), PointField::FLOAT32);
        add_field("tag", points_msg->fields.back().offset + sizeof(float), PointField::UINT8);
        add_field("line", points_msg->fields.back().offset + sizeof(uint8_t), PointField::UINT8);
    ...
    PointCloud2ConstPtr convert(const CustomMsg &livox_msg){
        ...
        for(int i = 0; i < livox_msg.point_num; ++i){
            ...
            *reinterpret_cast<uint32_t*>(ptr + points_msg->fields[3].offset) = livox_msg.points[i].offset_time;
            *reinterpret_cast<float*>(ptr + points_msg->fields[4].offset) = livox_msg.points[i].reflectivity;
            *reinterpret_cast<uint8_t*>(ptr + points_msg->fields[5].offset) = livox_msg.points[i].tag;
            *reinterpret_cast<uint8_t*>(ptr + points_msg->fields[6].offset) = livox_msg.points[i].line;
